...
extension UITableView{
    func checkAndDownloadProductImage(user: Product, vc: UIViewController, picID: String, cell: ProductCell?, index: Int, type: String?){
        ...
        if let imgFromCache = cache.imageFromCache(forKey: picID){
            ...
            if !tokens.contains(where: {$0 == picID}){
                tokens.append(picID)
                print("\(user.name ?? "") : \(user.blurred ?? false)")

                downloadProductImage(pictureProduct: cell, followingUID: user.uid, picID: picID, index: index, downloader: downloader, feedVC: vc as? FeedVC, friendVC: vc as? FriendVC, userVC: vc as? UserVC, type: type, product: user){
    ...
    func downloadProductImage(pictureProduct: ProductCell?, followingUID: String, picID: String, index: Int, downloader: SDWebImageDownloader?, feedVC: FeedVC?, friendVC: FriendVC?, userVC: UserVC?, type: String?, product: Product?, completed: @escaping () -> ()){
